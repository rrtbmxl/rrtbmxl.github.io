...
这一部分收集了跟机器学习相关的文章和教程，虽然对这个领域很感兴趣，但目前还没有达到入门水平，希望哪天能有机会用到，而不是仅仅局限在看教程的阶段。
...
### Basics
 ...
 - [Differences Between Machine Learning & Deep Learning](https://www.datacamp.com/community/tutorials/machine-deep-learning) (190327)  
 介绍了机器学习和深度学习相关的算法、应用以及两者之间的差别。
...
### Clustering 
 ...
 - [K-Means Clustering in R Tutorial](https://www.datacamp.com/community/tutorials/k-means-clustering-r?utm_campaign=News&utm_medium=Community&utm_source=DataCamp.com) (190103; 190319)  
 介绍了一些关于聚类的概念，然后以Uber的一份数据采用K均值聚类算法进行了演示。对无监督学习的理解还不够深入，所以有个别地方没有看明白。
 
 - [Using MCA and variable clustering in R for insights in customer attrition](https://datascienceplus.com/using-mca-and-variable-clustering-in-r-for-insights-in-customer-attrition/) (190321)  
 使用Multiple correspondence analysis方法和Variable clustering方法对顾客流失的影响因素进行了探索。虽然两种方法都是第一次听说，但看下来，也基本上都能明白，就是最后的代码写的太乱，有时间用tidyverse风格重写一遍看看。
 
 - [Cluster Analysis - Part 1: Introduction](https://www.inwt-statistics.com/read-blog/cluster-analysis-introduction.html) (190325);  
 [Cluster Analysis - Part 2: Hands On](https://www.inwt-statistics.com/read-blog/cluster-analysis-hands-on.html) (190325)  
 第一部分介绍了聚类的基本概念，第二部分使用k均值聚类对`mtcars`数据集进行了聚类分析。
 
 - [UTILIZING K-MEANS TO EXTRACT COLOURS FROM YOUR FAVOURITE IMAGES](https://chichacha.netlify.com/2018/11/12/utilizing-k-means-to-extract-colours-from-your-favourite-images/) (190331)  
 使用K均值聚类从图片中提取颜色，可以用来做palette，同时也介绍了一些关于处理颜色的方法。

### Deep Learning

 - [A “weird” introduction to Deep Learning](https://towardsdatascience.com/a-weird-introduction-to-deep-learning-7828803693b0) (190324)  
 介绍了深度学习的一些理论、应用和资源，但愿哪天能用上。
 
 - [Understanding the Magic of Neural Networks](http://blog.ephorie.de/understanding-the-magic-of-neural-networks) (190401)  
 拿小红帽的故事介绍了神经网络，同时也附有一个真实案例的代码。
 
 - [The Most Intuitive and Easiest Guide for Artificial Neural Network](https://towardsdatascience.com/the-most-intuitive-and-easiest-guide-for-artificial-neural-network-6a3f2bc0eecb) (190402)；  
 [The Most Intuitive and Easiest Guide for Convolutional Neural Network](https://towardsdatascience.com/the-most-intuitive-and-easiest-guide-for-convolutional-neural-network-3607be47480) (190402);  
 [The Most Intuitive and Easiest Guide for Recurrent Neural Network](https://towardsdatascience.com/the-most-intuitive-and-easiest-guide-for-recurrent-neural-network-873c29da73c7) (190402)  
 介绍神经网络的一系列文章，第一篇介绍了人工神经网络，文章下面有上面那篇文章作者的留言；第二篇关于卷积神经网络的，缺一些必要知识，没有完全看懂；第三篇介绍循环神经网络，大部分都没看懂。
 
 - [Neural Network Models in R](https://www.datacamp.com/community/tutorials/neural-network-models-r) (190403)  
 介绍了神经网络的一些基本概念，并带有一个很简单的例子。看过上面的三篇文章后，这篇就很简单的，而且发现上上篇文章的作者，也在这里留言了......
 
 - [Want to know how Deep Learning works? Here’s a quick guide for everyone.](https://medium.freecodecamp.org/want-to-know-how-deep-learning-works-heres-a-quick-guide-for-everyone-1aedeca88076) (190403)  
 介绍了机器学习和深度学习的一些概念，以及一些学习资源，获得了40K的claps。

### Dimensionality Reduction

- [DIMENSIONALITY REDUCTION METHODS USING FIFA 18 PLAYER DATA](http://blog.schochastics.net/post/dimensionality-reduction-methods/) (190329)  
使用游戏FIFA18的球员数据，介绍了PCA、t-SNE和SOM三种降维方法。

### Prediction

 - [Linear Regression from Scratch in R](https://datascienceplus.com/linear-regression-from-scratch-in-r/) (190311)  
 本以为这篇文章是系统介绍线性回归的，实际上只说了多元线性模型中那些β参数的值是怎么算出来的。然而看完我也不知道为什么那个公式能把那些参数算出来，看来还要补一些数学知识。
 
 - [A tutorial on tidy cross-validation with R](https://www.brodrigues.co/blog/2018-11-25-tidy_cv/) (190328)  
 使用随机森林算法对波士顿的房价进行了预测，介绍了`tidymodels`的使用以及使用贝叶斯优化超参数的方法。
 
 - [Regularization: Ridge, Lasso and Elastic Net](https://www.datacamp.com/community/tutorials/tutorial-ridge-lasso-elastic-net) (190404)  
 看完ASL的相关章节后看的这篇教程，能看懂一点概念，但数学和代码就看不懂了。
...
### Function
 ...
 - [Adding strings in R](https://jcarroll.com.au/2018/10/06/adding-strings-in-r/)（190303）通过编写一个用于连接字符的`+`函数，引出了很多有意思（而且看不太懂）的话题。
...
### Onlinebook
这一模块收集了与R语言相关的在线书籍。通过读书学习，我们至少能又两种收获，一是把已觉知的未知转变为已知，二是把未觉知的未知转变为已觉知的未知，这两种收获在学习中都非常重要。《老子》中说：“为学日益，为道日损”，如果把两种收获分别视为一个分数的分子和分母的话，那为学日益，是增加了分子上的已知，即“有”，而为道日损，则是增加了分母上的未知，即“无”，当“损之又损”，将所有未知都纳入自己的觉知范围之内时，大概就能“无为”了吧。
...
挺久以前看的了，当时还没有完成，而且以前似乎不是叫现在这个名字。感觉没有上面那本书好，但有一章介绍了一些绘图的理论，值得一看。
 ...
 - [Data Science Live Book](https://livebook.datascienceheroes.com/)（181218）一个R包的文档扩展成的书，排版有点乱，不推荐看。
 ...
 - [R in Action, Second Edition](https://www.manning.com/books/r-in-action-second-edition?a_bid=5c2b1e1d&a_aid=RiA2ed)（190221）《R语言实战》英文第二版的在线版，可以免费看。
 ...
 - [Advanced R ](http://adv-r.had.co.nz/) (190403)  
 鼓起勇气看了一遍，难度相当大，但也开阔了视角。
...
### Website
 ...
 - [100 FREE TUTORIALS FOR LEARNING R](https://www.listendata.com/p/r-programming-tutorials.html) (190313)  
 罗列的80来篇教程，全看了一遍，内容上有重复，前面44篇介绍了R的基本操作，偶有收获，但大部分比较过时，后面介绍了很多模型的实例，挺详细的，但缺点是排版不好，往往是先讲一大堆理论，再列一大堆代码，没有实际去跑，真跑的话，体验肯定不好。

# Visualization

与可视化相关的内容，以与`ggplot2`相关内容为主。
